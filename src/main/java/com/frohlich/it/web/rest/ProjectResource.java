...
public class ProjectResource {
    ...
    */
    ...
    /**
     * POST  /projects/:id : suspend the "id" project.
     *
     * @param id the id of the projectDTO to delete
     * @return the ResponseEntity with status 200 (OK)
     **/

     @PostMapping("/projects/{id}/suspend")
     @Timed
     public ResponseEntity<Void> suspendProject(@PathVariable Long id) {
         log.debug("REST request to suspend Project : {}", id);

         if (id == null) {
             throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
         }

         projectService.suspend(id);

         return ResponseEntity.ok()
             .headers(HeaderUtil
                 .createAlert("A " + ENTITY_NAME + " is suspended with identifier " + id.toString(), id.toString()))
             .build();
     }
